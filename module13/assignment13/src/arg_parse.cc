...
Args::Args() : input_filename_(), help_printed_(false), print_intermediate_steps_(false) {}
...
bool Args::ParseArgs(int argc, char **argv) {
  ...
  while (argc > 0) {
    if (argc > 1 && (std::strcmp(argv[0], "-i")  == 0 || std::strcmp(argv[0], "--input") == 0)) {
      input_filename_ = argv[1];
      --argc;
      ++argv;
    } else if (std::strcmp(argv[0], "-p") == 0 || std::strcmp(argv[0], "--print") == 0) {
      print_intermediate_steps_ = true;
    } else if (std::strcmp(argv[0], "-h") == 0 || std::strcmp(argv[0], "--help") == 0) {
      PrintUsage();
      help_printed_ = true;
      return true;
    } else {
...
void Args::PrintUsage() {
  std::cout << "Usage: [-h | --help] [-i <input_csv> | --input <input_csv>] [-p | --print]\n"
